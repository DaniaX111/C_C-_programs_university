...
void insertRectangle(struct rectangle **arr, int *n) {
    struct rectangle *new_arr = realloc(*arr, (*n + 1) * sizeof(struct rectangle));
    if (new_arr == NULL) {
...
// Вставка прямоугольника по индексу с использованием динамической памяти
void insertRectangleAtIndex(struct rectangle **arr, int *n, int index) {
    ...
    struct rectangle *newArr = realloc(*arr, (*n + 1) * sizeof(struct rectangle));
    if (newArr == NULL) {
        printf("Ошибка выделения памяти.\n");
        return;
    }
...
void deleteRectangle(struct rectangle **arr, int *n, int index) {
    ...
    (*n)--;
    struct rectangle *new_arr = realloc(*arr, (*n) * sizeof(struct rectangle));
    if (*n > 0 && new_arr == NULL) {
